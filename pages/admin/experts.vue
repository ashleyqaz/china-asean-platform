<template>
  <div class="admin-shell"><AdminSidebar /><main class="main">
    <div class="head"><h1>专家管理</h1><button class="btn" @click="openForm()">新增专家</button></div>
    <table class="tbl"><thead><tr><th>ID</th><th>姓名</th><th>职称</th><th>单位</th><th>研究领域</th><th>操作</th></tr></thead><tbody><tr v-for="e in items" :key="e.id"><td>{{e.id}}</td><td>{{e.name}}</td><td>{{e.title}}</td><td>{{e.organization}}</td><td>{{e.field}}</td><td><button class="sm" @click="openForm(e)">编辑</button><button class="sm del" @click="del(e.id)">删除</button></td></tr></tbody></table>
    <div v-if="show" class="modal-overlay" @click.self="show=false"><div class="modal"><h3>{{editId?'编辑':'新增'}}</h3><div class="fg"><label>姓名</label><input v-model="f.name" class="inp"></div><div class="fg"><label>职称</label><input v-model="f.title" class="inp"></div><div class="fg"><label>头像URL</label><input v-model="f.avatar_url" class="inp"></div><div class="fg"><label>单位</label><input v-model="f.organization" class="inp"></div><div class="fg"><label>研究领域</label><input v-model="f.field" class="inp"></div><div class="fg"><label>简介</label><textarea v-model="f.bio" class="inp" rows="3"></textarea></div><div class="acts"><button class="btn" @click="save">保存</button><button class="btn-cancel" @click="show=false">取消</button></div></div></div>
  </main></div>
</template>
...
const f = reactive({ name:'', title:'', avatar_url:'', organization:'', field:'', bio:'', sort_order:0 })
async function load(){ try { const r = await api.get('/experts'); if(r.code===200) items.value=r.data } catch{}}
function openForm(e){ if(e){ editId.value=e.id; Object.keys(f).forEach(k=>{ if(e[k]!==undefined) f[k]=e[k] }); f.avatar_url=e.avatarUrl||'' } else { editId.value=null; Object.assign(f,{name:'',title:'',avatar_url:'',organization:'',field:'',bio:'',sort_order:0}) } show.value=true }
async function save(){ try { editId.value?await api.put(`/experts/${editId.value}`,{...f}):await api.post('/experts',{...f}); show.value=false; load() } catch{} }
async function del(id){ if(!confirm('确定删除？'))return; await api.del(`/experts/${id}`); load() }
onMounted(load)
</script>
<style scoped>
